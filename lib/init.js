...
const uiVerificationLine = "如果本需求需要浏览器 UI 验证，请在 spec.md 中单独一行写入：uiVerification: required";
...
function updateProject(root) {
  ...
  const changes = [];
  const uiVerifyFile = path.join(agentsDir(root), "ui-verify.md");
  if (fs.existsSync(uiVerifyFile)) {
    changes.push("kept agents/ui-verify.md");
  }

  const designFile = path.join(agentsDir(root), "design.md");
  if (appendIfMissing(designFile, uiVerificationLine, `\n补充要求：\n- ${uiVerificationLine}\n`)) {
    changes.push("patched agents/design.md");
  } else {
    changes.push("kept agents/design.md");
  }
